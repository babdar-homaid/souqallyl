...
<?php
  ...
  $id = $_GET['id'];
  $sql = "Select * from assignee where id = $id";
  $rs = $db->dbQuery($sql);
  ...
  if(isset($_POST['btnAdd'])){
     $name = $_POST['name'];
         ...
         $sql = "UPDATE `assignee` SET `name` = '$name' WHERE `assignee`.`id` = $id";
         $rs = $db->dbQuery($sql);
         $err = "تم التعديل بنجاح";

         echo "<meta http-equiv='refresh' content='3;URL=assignee.php'>";
    ...
    <div class="dad">
      ...
      <form method="post">
        <div class="form-group">
          <label >تعديل جهة</label>
          <input type="text" class="form-control input" name="name" value="<?= $row[1] ?>"  placeholder="تعديل جهة  ">
        </div>
